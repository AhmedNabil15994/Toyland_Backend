...
class CatalogRepository
{
    ...
    public function getProductsByCategory($request)
    {
        ...
        $products = $this->prd->active()/* ->whereNotNull('branch_id')
            ->where('branch_id', auth()->user()->branch_id) */->with([
        ...
        return $products->orderBy('id', 'DESC')->paginate(30);
    }
    ...
    public function productSearch($model, $request)
    {
        ...
        return $model->where(function ($query) use ($term) {
            ...
            $query->orWhereRaw('lower(international_code) like (?)', ["%{$term}%"]);
