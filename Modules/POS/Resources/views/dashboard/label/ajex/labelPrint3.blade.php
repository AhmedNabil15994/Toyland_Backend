<div>
    ...
    <div class="printer-container"  >
            ...
            @if($productRequest)
                ...
                @if($productRequest["num"] >= 1)
                    @foreach (range(1,$productRequest["num"] ) as $counterVariation)
                        ...
                        {{-- Actual Paper --}}
                            <div style="
                            @if(!$barcode_details->is_continuous) height:{{$barcode_details->paper_height}}in !important; @endif width:{{$barcode_details->paper_width}}in !important; line-height: 16px !important;
                            @if(!$barcode_details->is_continuous)padding-top:{{$barcode_details->top_margin}}in !important; padding-bottom:{{$barcode_details->top_margin}}in !important; padding-left:{{$barcode_details->left_margin}}in !important;padding-right:{{$barcode_details->left_margin}}in !important;@endif
                            "
                            class=" label-border-outer @if(!$request->setSizePage && ($total_qty >  $barcode_details->stickers_in_one_sheet + $loop_count)) page-break  @endif">
                        @endif

                        @if(is_new_row($barcode_details, $loop_count))
                            {{-- Paper Internal --}}
                            <div
                                style=""
                       ...
                       @if (is_end_row($barcode_details, $loop_count))
                           {{-- Paper Internal --}}
                        </div>
                        ...
                        @if($variationRequest["num"] >= 1)
                            ...
                            @if($varaition)
                                    @foreach (range(1,$variationRequest["num"] ) as $counterVariation)
                                        ...
                                        @if (is_new_paper($barcode_details, $loop_count))
                                        {{-- Actual Paper --}}
                                            <div style="@if(!$barcode_details->is_continuous) height:{{$barcode_details->paper_height}}in !important; @endif width:{{$barcode_details->paper_width}}in !important; line-height: 16px !important;" class=" label-border-outer  @if(!$request->setSizePage && ($total_qty >  $barcode_details->stickers_in_one_sheet + $loop_count)) page-break  @endif ">
                                        @endif
        ...
        @if($barcode_details->is_continuous  || !is_paper_end($barcode_details, $loop_count))
            {{-- Actual Paper --}}
            </div>
        @endif
...
<style>
    ...
    @page {
		...
		size: {{$barcode_details->paper_width}}in @if( !$barcode_details->is_continuous && $barcode_details->paper_height ){{$barcode_details->paper_height}}in @else {{$paper_height}}in @endif;
        @endif
